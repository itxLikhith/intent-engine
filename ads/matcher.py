...
from core.schema import (
    UniversalIntent, Constraint, ConstraintType, UseCase, EthicalSignal,
    EthicalDimension, IntentGoal, DeclaredIntent, InferredIntent
)
...
class AdFairnessChecker:
    ...
    def __init__(self):
        ...
        self.forbidden_dimensions = {
            'age', 'gender', 'income', 'race', 'ethnicity', 'political_affiliation',
            'religious_belief', 'health_condition', 'sexual_orientation', 
            'location', 'behavior', 'interests', 'purchasing_history',
            'browsing_history', 'device_fingerprint', 'ip_address'
        }
...
class AdRelevanceScorer:
    ...
    def _compute_semantic_similarity(self, ad: AdMetadata, intent: UniversalIntent, declared: DeclaredIntent) -> Tuple[float, List[str]]:
    ...
    def _compute_ethical_alignment(self, ad: AdMetadata, intent: UniversalIntent, inferred: InferredIntent) -> Tuple[float, List[str]]:
    ...
    def _compute_goal_alignment(self, ad: AdMetadata, intent: UniversalIntent, declared: DeclaredIntent) -> Tuple[float, List[str]]:
...
class AdMatcher:
    ...
    def match_ads(self, request: AdMatchingRequest) -> AdMatchingResponse:
        ...
        logger.info(f"Ad matching started: {total_ads_evaluated} ads, query='{declared.query[:50] if declared.query else 'N/A'}...'")
        ...
        for idx, ad in enumerate(request.adInventory):
            ...
            relevance_score, relevance_reasons = self.relevance_scorer.compute_ad_relevance(
                ad,
                request.intent
            )
            ...
            if relevance_score > min_threshold:
                ...
                matched_ad = MatchedAd(
                    ad=ad,
                    adRelevanceScore=relevance_score,
                    matchReasons=all_reasons
                )
