...
class PerformanceBenchmark:
    ...
    async def single_request_latency(self, session: aiohttp.ClientSession, query: str) -> dict[str, Any]:
        ...
        try:
            async with session.post(SEARCH_ENDPOINT, json=payload, timeout=aiohttp.ClientTimeout(total=TIMEOUT_SECONDS)) as response:
    ...
    async def concurrent_request(
        ...
        tasks = [
            self.single_request_latency(session, query)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, query in enumerate(TEST_QUERIES, 1):
                ...
                for j in range(5):
                    ...
                    print(f"  Request {j+1}: {result['response_time_ms']}ms "
                          f"({result['results_count']} results)", flush=True)
            ...
            for num_users in CONCURRENT_USERS:
                ...
                if latencies:
                    load_result = {
                        ...
                        "latency_p95_ms": round(sorted(latencies)[int(len(latencies) * 0.95)] if len(latencies) > 20 else max(latencies), 2),
                    }
                    ...
                    print(f"  ✓ Latency: {load_result['latency_mean_ms']}ms avg, "
                          f"{load_result['latency_p95_ms']}ms p95")
                else:
    ...
    def _print_summary(self):
        ...
        for load in self.results["concurrent_load"]:
            if "error" not in load:
                print(f"  {load['concurrent_users']:<8} {load['requests_per_second']:<10} "
