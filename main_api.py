...
from database import db_manager, Ad as DbAd, AdMetric as DbAdMetric, Campaign as DbCampaign, AdGroup as DbAdGroup, CreativeAsset as DbCreativeAsset, Advertiser as DbAdvertiser, ClickTracking as DbClickTracking, ConversionTracking as DbConversionTracking, FraudDetection as DbFraudDetection, Base, engine
from models import (
    RankingRequest, RankingResponse,
    ServiceRecommendationRequest, ServiceRecommendationResponse,
    AdMatchingRequest, AdMatchingResponse,
    HealthCheckResponse, StatusResponse,
    CampaignCreate, Campaign, CampaignUpdate,
    AdGroupCreate, AdGroup, AdGroupUpdate,
    CreativeAssetCreate, CreativeAsset, CreativeAssetUpdate,
    AdCreate, Ad, AdUpdate,
    AdvertiserCreate, Advertiser,
    CampaignPerformanceReport,
    AdMatchingWithCampaignRequest,
    ClickTrackingCreate, ClickTracking,
    ConversionTrackingCreate, ConversionTracking,
    FraudDetectionCreate, FraudDetection,
    ConsentRecord, ConsentRequest, ConsentSummary,
    AuditEvent, AuditEventRequest, AuditStats,
    DataRetentionPolicy, PrivacyComplianceReport,
    ABTestCreate, ABTestUpdate, ABTestResponse, ABTestResultsResponse,
    ABTestVariantCreate, ABTestVariantResponse,
    FraudAnalysisResponse, FraudScanSummary,
    AttributionResultResponse, CampaignROIResponse, TrendAnalysisResponse,
    URLRankingAPIRequest, URLRankingAPIResponse, URLRankedResult,
...
from ranking.optimized_url_ranker import rank_urls, URLRankingRequest, URLRankingResponse as InternalURLRankingResponse
...
from privacy_core import validate_advertiser_constraints, anonymize_intent_data, is_intent_expired
from privacy.enhanced_privacy import get_enhanced_privacy_controls, DataRetentionPeriod, PrivacyControlType
from privacy.consent_manager import get_consent_manager, ConsentType, ConsentStatus
from audit.audit_trail import get_audit_trail_manager, AuditEventType
...
def convert_dict_to_universal_intent(intent_dict: Dict[str, Any]) -> UniversalIntent:
    ...
    from core.schema import (
        UniversalIntent, DeclaredIntent, InferredIntent, SessionFeedback,
        Constraint, TemporalIntent, EthicalSignal,
        IntentGoal, Urgency, SkillLevel, UseCase, ResultType, Complexity,
        ConstraintType, TemporalHorizon, Recency, Frequency, EthicalDimension
    )
    ...
    for es in (inferred_dict.get('ethicalSignals', []) or []):
        if isinstance(es, dict):
            ...
            ethical_signals.append(EthicalSignal(
...
service_recommendation_requests = Counter('service_recommendation_requests_total', 'Number of service recommendation requests')
...
async def startup_event():
    ...
    from abtesting.service import ABTest, ABTestVariant, ABTestAssignment
...
async def rank_urls_endpoint(request: URLRankingAPIRequest):
    ...
    try:
        from ranking.url_ranker import (
            URLRankingRequest as InternalURLRankingRequest,
            rank_urls,
        )
...
async def recommend_services_endpoint(request: ServiceRecommendationRequest):
    ...
    try:
        ...
        internal_request = InternalServiceRequest(
            intent=request.intent,
            availableServices=services_metadata,
            options=request.options
        )
...
async def match_ads_endpoint(request: AdMatchingRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        for req_ad in request.ad_inventory:
            ...
            if not compliance_report['is_compliant']:
                # Log violations but still include ad (would be filtered in production)
                logger.warning(f"Ad {req_ad.get('id', 'unknown')} has compliance violations: {compliance_report['violations']}")
                fairness_violations.inc(len(compliance_report['violations']))
        ...
        internal_request = InternalAdRequest(
            intent=anonymized_intent,
            adInventory=ad_inventory,
                    ...
                    if not active_campaigns:
                        ...
                        for campaign in active_campaigns:
                            try:
                                ...
                                campaign_spent = db.query(
                                    func.coalesce(func.sum(DbAdMetric.impression_count * DbAd.bid_amount), 0)
                                ).select_from(DbAdMetric)\
                                 .join(DbAd, DbAdMetric.ad_id == DbAd.id)\
                                 .join(DbAdGroup, DbAd.ad_group_id == DbAdGroup.id)\
                                 .filter(DbAdGroup.campaign_id == campaign.id).scalar() or 0
...
async def log_audit_event(
    ...
    audit_entry = audit_manager.log_event(
        ...
        action_description, ip_address, user_agent, metadata
    )
...
async def get_conversion_attribution(
    ...
    from analytics.advanced import get_advanced_analytics, AttributionModel
