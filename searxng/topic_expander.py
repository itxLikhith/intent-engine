...
class ReliableTopicExpander:
    ...
    async def add_search_query(self, query: str):
        ...
        for keyword in keywords:
            await self.redis_client.zincrby(self.keyword_stats_key, 1, keyword.lower())

        # Update category stats
        category = query_data["category"]
        await self.redis_client.hincrby(self.category_stats_key, category, 1)

        # Update total count
        self.stats["total_queries_processed"] += 1

        # Trim old data (keep last 10000 queries)
        await self.redis_client.zremrangebyrank(self.query_history_key, 0, -10001)
    ...
    def _categorize_keyword(self, keyword: str) -> str | None:
    ...
    async def _persist_topics(self):
        ...
        await self.redis_client.set(self.topics_key, json.dumps(data))
