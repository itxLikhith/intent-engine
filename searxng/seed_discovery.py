...
from searxng.query_router import UnifiedQueryRouter, QueryRoute, SearchBackend
...
class SeedURLDiscovery:
    ...
    async def add_to_crawler(
        self,
        urls: List[Dict],
        crawler_api_url: str = "http://go-search-api:8080"
    ) -> bool:
        """
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{crawler_api_url}/api/v1/crawl/seed",
                    json=payload,
...
async def run_seed_discovery():
    ...
    go_urls = await discovery.discover_multiple_topics(
        topics=DISCOVERY_TOPICS["go_language"],
        urls_per_topic=10
    )
    ...
    prog_urls = await discovery.discover_multiple_topics(
        topics=DISCOVERY_TOPICS["programming"],
        urls_per_topic=5
    )
