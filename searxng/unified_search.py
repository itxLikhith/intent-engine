...
from searxng.client import get_searxng_client, SearXNGResult
from extraction.extractor import extract_intent, IntentExtractionRequest
from ranking.url_ranker import rank_urls, URLRankingRequest
from models import (
...
class UnifiedSearchService:
    ...
    async def search(self, request: UnifiedSearchRequest) -> UnifiedSearchResponse:
        ...
        logger.info(f"Unified search: query='{request.query}', extract_intent={request.extract_intent}, rank_results={request.rank_results}")
            ...
            if isinstance(intent_result, Exception):
                ...
                if intent_result and hasattr(intent_result, 'intent'):
                    ...
                    if universal_intent:
                        ...
                        logger.debug(f"extracted_intent converted: goal={extracted_intent.goal if extracted_intent else 'None'}")
                else:
        ...
        if request.min_privacy_score or request.exclude_big_tech:
            ...
            ranked_results = self._apply_privacy_filters(
                ranked_results,
                request
            )
    ...
    async def _search_searxng(self, request: UnifiedSearchRequest) -> List[SearXNGResult]:
        ...
        try:
            ...
            logger.debug(f"_search_searxng: got {len(response.results) if response else 0} results from SearXNGResponse")
