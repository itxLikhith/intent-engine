...
class SearXNGClient:
    ...
    def __init__(self, base_url: str = None, redis_host: str = None, redis_port: int = 6379):
        ...
        logger.info(f"SearXNG client initialized with base URL: {self.base_url}, timeout: {self.timeout}s, connection pooling enabled")
    ...
    async def search(
        ...
        try:
            ...
            data = response.json()
            logger.debug(f"SearXNG response: query={data.get('query')}, results_count={len(data.get('results', [])) if data.get('results') else 'None'}")

            # Parse results - handle None case
            raw_results = data.get("results")
            if raw_results is None:
                logger.warning(f"SearXNG returned None for results field, data keys: {list(data.keys()) if data else 'None'}")
                raw_results = []
