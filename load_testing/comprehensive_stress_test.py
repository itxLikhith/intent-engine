...
class TestResults:
    ...
    def calculate_stats(self):
        ...
        if self.response_times:
            ...
            self.p95_response_time = sorted(self.response_times)[int(len(self.response_times) * 0.95)] if len(self.response_times) > 20 else self.max_response_time
            self.p99_response_time = sorted(self.response_times)[int(len(self.response_times) * 0.99)] if len(self.response_times) > 100 else self.max_response_time
...
class ComprehensiveStressTestSuite:
    ...
    async def test_concurrent_all_endpoints(self, concurrency: int = 100, duration: int = 60) -> TestResults:
        ...
        async def make_request(session, request_id: int):
            try:
                query = self.test_queries[request_id % len(self.test_queries)]
                endpoint = random.choice(["extract-intent", "rank-urls", "match-ads", "recommend-services", "campaigns"])
                ...
                if endpoint == "extract-intent":
                    ...
                    if endpoint == "match-ads":
                        ...
                        payload = {
                            "intent": intent,
                            "available_services": [
                                {"id": "search", "name": "Search", "supportedGoals": ["find_information"], "primaryUseCases": ["learning"]},
                                {"id": "docs", "name": "Docs", "supportedGoals": ["learn"], "primaryUseCases": ["learning"]}
                            ]
                        }
    ...
    def generate_report(self, output_file: str = "stress_test_report.json"):
        ...
        report = {
            ...
            "summary": {
                ...
                "overall_success_rate": sum(r.successful_requests for r in self.all_results) / 
                                       sum(r.total_requests for r in self.all_results) * 100 if self.all_results else 0,
