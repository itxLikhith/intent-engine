...
BOT_USER_AGENT_PATTERNS = [
    ...
    r'curl', r'wget', r'python-requests', r'httpx',
...
class FraudDetector:
    ...
    def _check_click_velocity(self, click_data: Dict[str, Any]) -> List[FraudSignal]:
        ...
        clicks_per_minute = self.db.query(DbClickTracking).filter(
        ...
        clicks_per_hour = self.db.query(DbClickTracking).filter(
            DbClickTracking.ip_hash == ip_hash,
        ...
        if session_id:
            session_clicks_per_minute = self.db.query(DbClickTracking).filter(
        ...
        if ad_id:
            ad_clicks_per_day = self.db.query(DbClickTracking).filter(
                DbClickTracking.ip_hash == ip_hash,
    ...
    def _check_ip_anomalies(self, click_data: Dict[str, Any]) -> List[FraudSignal]:
        ...
        unique_user_agents = self.db.query(
            func.count(func.distinct(DbClickTracking.user_agent_hash))
        ).filter(
            DbClickTracking.ip_hash == ip_hash,
            DbClickTracking.timestamp >= one_hour_ago
        ).scalar() or 0
    ...
    def _check_click_patterns(self, click_data: Dict[str, Any]) -> List[FraudSignal]:
        ...
        recent_clicks = self.db.query(DbClickTracking).filter(
            DbClickTracking.ip_hash == ip_hash,
            DbClickTracking.timestamp >= five_seconds_ago
        ).order_by(DbClickTracking.timestamp.desc()).limit(10).all()
    ...
    def _check_conversion_patterns(self, conversion_data: Dict[str, Any]) -> List[FraudSignal]:
        ...
        existing_conversions = self.db.query(DbConversionTracking).filter(
            DbConversionTracking.click_id == click_id
        ).count()
    ...
    def run_batch_analysis(self, hours: int = 24) -> Dict[str, Any]:
        ...
        """
        from database import ClickTracking as DbClickTracking, FraudDetection as DbFraudDetection
        
        start_time = datetime.utcnow() - timedelta(hours=hours)
        
        # Analyze recent clicks
        recent_clicks = self.db.query(DbClickTracking).filter(
            DbClickTracking.timestamp >= start_time
        ).all()
